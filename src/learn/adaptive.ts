...
export interface Diagnosis {
  daysSinceLastVisit: number | null;  // 가장 최근 진척 기록으로부터 며칠 지났는지(기록 없으면 null=신규)
...
// 진척 전체에서 가장 최근 lastSeenAt을 찾아 "며칠 전"인지 환산 — 며칠 만에 재접속했는지 판단용.
// 기록이 하나도 없으면(신규 유저) null — "복습할 게 없으니 며칠 지남" 메시지를 잘못 띄우지 않게.
function daysSince(progress: ProgressMap): number | null {
  let latest = 0;
  for (const p of Object.values(progress)) {
...
export function diagnose(allCards: Card[], progress: ProgressMap, lastCompletedSessionId: number): Diagnosis {
  ...
  const gap = daysSince(progress);

  // 오늘 집중 포인트 — 우선순위: 재접속 공백(며칠 만인지) > 장면 > 기술 > 글자 > 일반.
  // 며칠 비웠다 오면 새 표현 확장보다 "그동안 배운 걸 다시 다지기"가 먼저여야 한다.
  let focus: string;
  if (gap !== null && gap >= 7) focus = '오랜만이에요 — 복습으로 감 되찾기';
  else if (gap !== null && gap >= 2) focus = `${gap}일 만이에요 — 복습부터 가볍게`;
  else if (weakScenes.length) focus = `${weakScenes[0].label} 장면 다지기`;
  ...
  // 튜터 코멘트 — 재접속 공백이 있으면 최우선(성과 기준보다 앞섬), 없으면 최근 성과 기준.
  let message: string;
  if (gap !== null && gap >= 7) message = `${gap}일 만이네요! 오늘은 새 표현보다 그동안 배운 걸 다시 다지는 게 먼저예요.`;
  else if (gap !== null && gap >= 2) message = `${gap}일 만이에요. 감 잃지 않게 오늘은 복습 비중을 늘렸어요.`;
  else if (level === 'struggling') message = '최근 정답률이 낮아요. 새 항목을 줄이고 약점을 더 반복합니다.';
  ...
  return { daysSinceLastVisit: gap, seen, recentAccuracy: acc, recentSample: n, level, weakKana, weakScenes, weakSkill, focus, message };
}
...
export function adaptSessionConfig(base: SessionConfig, diag: Diagnosis): { config: SessionConfig; changed: boolean } {
  ...
  // 며칠 만에 재접속 — 공백이 길수록 새 항목 최소치를 더 낮춰 복습(익힌 것·오래된 것) 비중을 그만큼 늘린다.
  // quotas(세션 총량)는 그대로라 세션이 줄지 않고, minFresh만 낮아져 due 카드가 있으면 그 자리를 채운다.
  const gap = diag.daysSinceLastVisit;
  if (gap !== null && gap >= 7) {
    minFresh.K = 0; minFresh.B = 0; minFresh.C = 0;
    changed = true;
  } else if (gap !== null && gap >= 2) {
    minFresh.K = Math.max(0, Math.min(minFresh.K, base.minFresh.K - 1));
    minFresh.C = Math.max(0, Math.min(minFresh.C, base.minFresh.C - 1));
    changed = true;
  }
  return { config: { quotas: base.quotas, minFresh, missionTierFilter: base.missionTierFilter, cardTierRange: base.cardTierRange, openMissions: base.openMissions }, changed };
}
