...
//   ③ 레벨별 10런 스트레스 — 실제 MODE_PRESETS 설정·랜덤 응답에서 학습선행 위반 0 + 하한 미만 세션 노출 0.
//   ④ 각 레벨은 자기 tier 범위(중급 tier2~5·고급 tier3~5)를 10런 합집합으로 전부 커버.
...
console.log(`\n=== ② 승급 여정(입문→기본→중급→고급) ${RUNS}런 — 모든 미션 도달 가능 + 학습선행 0 ===`);
{
  ...
  for (let r = 0; r < RUNS; r++) {
    ...
    for (const mode of ['beginner', 'default', 'express', 'advanced'] as LearnMode[]) {
      const res = playLevel(mode, 3000 + r * 211 + mode.length, 70, sid, progress, open, m);
  ...
  check(`${RUNS}런 합집합이 50개 미션 전부 도달`, missed.length === 0, `누락 [${missed.join(',')}]`);
...
// 상위 레벨(중급 floor2·고급 floor3)은 자기 범위(floor~5)를 전부 도달해야 한다.
// 입문·기본(floor1)은 상위 tier를 승급으로 도달하므로 여기선 "하한~중간" 커버만 확인(설계상 tier5 미도달 정상).
