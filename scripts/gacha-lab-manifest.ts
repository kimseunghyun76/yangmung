...
// 랩 가챠는 아이템 이미지를 /gacha/items/generated-v2/<slug>.png 로 자동 해석하므로,
...
writeFileSync('docs/gacha-lab-assets.json', JSON.stringify({ generatedAt: new Date().toISOString().slice(0, 10), count: items.length, note: 'image 경로 그대로 투명 png(640²) 생성 → 랩 가챠/도감에 자동 반영', items }, null, 2) + '\n');
